...
-(BOOL) activateLicense{
    ...
    if (FSDKE_OK != FSDK_ActivateLibrary("TmDIVQ0h7sxGMkl6Euk2ABZHxkHPFoHc6bdYIeBpO38exkVpQ+kzLAltzn06HcrfrUIzp4q3evzb4zVQ0gLl8vZ5RyULBl4cjh0GIGQBsc3/Qptq5OIkJUj9O0K7zAJqnbjHaE4e8b6dehMwGxWOIypjouySEqeM6dCUOHkjN1Y="))
    {
...
- (void)processNewCameraFrame:(CVImageBufferRef)cameraFrame
{
    ...
    // Get the number of bytes per row for the pixel buffer
    u_int8_t *baseAddress = (u_int8_t *)malloc(bytesPerRow*bufferHeight);
    
    memcpy( baseAddress, CVPixelBufferGetBaseAddress(pixelBuffer), bytesPerRow * bufferHeight     );
    
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGContextRef context = CGBitmapContextCreate(baseAddress, bufferWidth, bufferHeight, 8,
                                                 bytesPerRow, colorSpace, kCGBitmapByteOrder32Little | kCGImageAlphaNoneSkipFirst);
    
    HImage himage;
    int res = FSDK_CreateEmptyImage(&himage);
    
    res = FSDK_LoadImageFromBuffer(&himage, baseAddress, bufferWidth, bufferHeight, (unsigned int)bytesPerRow, FSDK_IMAGE_COLOR_32BIT);
...
//
    ...
    if (FSDKE_OK != FSDK_LoadImageFromFile(&hFirstImage,"Foto.jpg"))
    {
        NSLog(@"File not found %s","Foto.jpg");
//        exit(-1);
    }
...
//    FSDK_MatchFaces(&FirstFaceTemplate,&SecondFaceTemplate,&fSmilarity);
    ...
    if (_processingImage == NO && faceTouched == NO) {
        ...
        int scanline = (int)CVPixelBufferGetBytesPerRow(pixelBuffer);
//        unsigned char * buffer = (unsigned char *)malloc(scanline * bufferHeight);
        
        unsigned char *buffer = (unsigned char*)malloc(sizeof(char) * bufferWidth * bufferHeight * 4);
        
        NSLog(@"sizeofbuffer%lu",sizeof(buffer));
        
        if (buffer) {
            uint8_t* destPixels = (uint8_t*)CVPixelBufferGetBaseAddress(pixelBuffer);
            memcpy(buffer, CVPixelBufferGetBaseAddress(pixelBuffer), scanline * bufferHeight);
            NSLog(@"sizeofbuffer%lu",sizeof(buffer));
        } else {
            _processingImage = NO;
//            glDeleteTextures(1, &videoFrameTexture);
        ...
        HImage image;
        int res = FSDK_CreateEmptyImage(&image);
         res = FSDK_LoadImageFromJpegBuffer(&image, buffer, scanline * bufferHeight);
...
//        [pool release];
    ...
    HImage imageComparison = 0;
    if (FSDKE_OK != FSDK_LoadImageFromFile(&imageComparison,"Imagen.jpg"))
    {
